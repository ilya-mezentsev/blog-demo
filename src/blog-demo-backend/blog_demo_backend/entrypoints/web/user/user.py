...
class UserEntrypoint:
    ...
    async def _read_users(self, request: web.Request) -> web.Response:
        response_model = await self._user_domain.user_service.read(GetUsersRequest(
            request_user_id=request['context']['user_id'],
        ))
